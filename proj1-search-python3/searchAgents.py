...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        nextTakenCorners = [0, 0, 0, 0]
        for corner, index in enumerate(self.corners):
            if self.startingPosition == corner:
                nextTakenCorners[index] = 1
        nextTakenCorners = tuple(nextTakenCorners)
        return self.startingPosition, nextTakenCorners

    def isGoalState(self, state):
        """
        Returns whether this search state is a goal state of the problem.
        """
        _, takenCorners = state
        if takenCorners == (1, 1, 1, 1):
            return True
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        currentPosition, takenCorners = state
        successors = []
        for action in [
            Directions.NORTH,
            Directions.SOUTH,
            ...
            nextTakenCorners = list(takenCorners)
            ...
            if not hitsWall:
                successors.append(
                    (((nextx, nexty), nextTakenCorners), action, 1)
                )
...
def cornersHeuristic(state, problem):
    ...
    for permute in itertools.permutations(cornersLeft):
        curr = util.manhattanDistance(currentPosition, permute[0])
